...
io.on("connection", function (socket) {
    ...
    pool.getConnection(function (err, conn) {
      ...
      conn.query(
        ...
        function (err, results) {
          ...
          let flag = 0;
          let allLight = new Array();
          let eleDate = new Array();
          for (var i = 0; i < 10; i++) {
            let light = 0;
            data[i] = new Array();
            for (var j = 0; j < 20; j++) {
              data[i][j] = results[flag];
              flag++;
            }
            for (let em of data[i]) {
              light = light + parseInt(em['light']);
            }
            let date = data[i][0]['date']; 
            date = RiQi(date)
            eleDate.push(date);
            let lightAverage = light / 20;
            let l = lightAverage * 0.001;
            allLight.push(l.toFixed(2));
          }
          let eleData = {
            "date": eleDate,
            "allLight": allLight
          ...
          socket.emit("eleChart", { msg: JSON.stringify(eleData) });
  ...
  client.on("message", function (topic, message) {
    ...
    if (topic == "Mymqtt/follow/willMessage") {
      ...
      var json = JSON.parse(message.toString());
      switchState = json.switch;
      console.log(switchState);

      socket.emit("electricity", { msg: json });
      socket.emit("SWITCH", { msg: switchState })
      nowTime = new Date().toLocaleTimeString("chinese", { hour12: false });
      if (time != nowTime) {
